...
- **Dual Relay Fallback Architecture:** If a direct P2P connection cannot be established (e.g., due to strict NATs or firewalls), the application employs a two-tier fallback strategy to ensure reliability:
  1. **WebRTC TURN Relay:** As a first resort, WebRTC natively attempts to route traffic through a configured TURN server (e.g., `free.expressturn.com`). The DataChannel connects successfully, but traffic is relayed at the WebRTC protocol layer.
  2. **WebSocket Relay Fallback:** If the WebRTC connection entirely fails or times out (e.g. within 10 seconds), the system abandons WebRTC completely. It falls back to an application-level relay over the existing WebSocket connection. File chunks are converted to Base64 and sent directly through the Node.js signaling server (`relay-meta`, `relay-chunk`, `relay-done`), ensuring the transfer completes even when WebRTC is completely blocked.
